...
//    private ContentResolver cr;
    ...
    private AlbumBitmapCacheHelper() {
        //分配1/4的运行时内存给图片显示
        final int memory = (int) (Runtime.getRuntime().maxMemory() / 1024 / 8);
...
//        cache.resize((int) (Runtime.getRuntime().maxMemory() / 1024 / 4));
     ...
     * 通过path获取图片bitmap
     */
    public void decodeBitmapFromPath(final Activity activity, final String path, final ILoadImageCallback callback, final Object... objects) throws OutOfMemoryError {
        ...
        Runnable thread = new Runnable() {
            ...
            public void run() {
                ...
                msg.obj = ImageUtils.compressImage(activity, path);
...
//    ExecutorService tpe = Executors.newFixedThreadPool(1);
     ...
     * 通过path获取图片bitmap
     */
    public void decodeBitmapFromPath(final Activity activity, final String path, final int width, final int height, final ILoadImageCallback callback, final Object... objects) throws OutOfMemoryError {
        final Handler handler = new Handler() {
            @Override
            public void handleMessage(Message msg) {
                callback.onLoadImageCallBack((Bitmap) msg.obj, path, objects);
        ...
        Runnable thread = new Runnable() {
            @Override
            public void run() {
            	Log.e("path", path + (!currentShowString.contains(path) || cache == null));
                ...
                if (width == 0 || height == 0) {
                    ...
                    options.inSampleSize = computeScale(options, activity.getWindowManager().getDefaultDisplay().getWidth(), activity.getWindowManager().getDefaultDisplay().getWidth());
                    ...
                    File file = new File(CommonUtil.getDataPath(activity.getBaseContext()));
                    if (!file.exists())
                        file.mkdirs();
                    //临时文件的文件名
                    String tempPath = CommonUtil.getDataPath(activity.getBaseContext()) + hash + ".temp";
...
//                    }
                ...
                Message msg = Message.obtain();
                msg.obj = ImageUtils.correctBitmap(bitmap, path);
                handler.sendMessage(msg);
            }
        };
